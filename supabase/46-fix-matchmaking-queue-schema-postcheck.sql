-- =============================================================================
-- MIGRACIÓN 46: POSTCHECK READ-ONLY AUDIT
-- =============================================================================
-- PROPÓSITO:
-- Auditoría estricta de sólo lectura tras aplicar la migración 46.
-- NO MODIFICA DATOS NI ESTRUCTURA.
-- =============================================================================

-- 1. VERIFICAR QUE LA FASE 46 ESTÁ REGISTRADA EN EL LEDGER
SELECT
  fase,
...
SELECT
  p.proname,
  pg_get_functiondef(p.oid) LIKE '%matched_room_id = v_new_room_id%' AS tiene_matched_room_id,
  pg_get_functiondef(p.oid) LIKE '%status = ''matched''%' AS tiene_status_matched,
  pg_get_functiondef(p.oid) NOT LIKE '%matched_at%' AS sin_matched_at,
...
-- 3. BASELINE COMPETITIVO POST-MIGRACIÓN 46
SELECT
